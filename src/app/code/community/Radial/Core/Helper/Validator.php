...
class Radial_Core_Helper_Validator
{
    const API_HANDLER_PATH = 'radial_core/api/test_connection_status_handlers';
    const INVALID_API_SETTINGS_ERROR_MESSAGE = 'Radial_Core_Api_Validation_Invalid_Settings';
    ...
    const INVALID_SFTP_SETTINGS_ERROR_MESSAGE = 'Radial_Core_Sftp_Validation_Invalid_Settings';
    const INVALID_SFTP_CONNECTION = 'Radial_Core_Sftp_Invalid_Connection';
    const INVALID_SFTP_AUTHENTICATION_CONFIG = 'Radial_Core_Sftp_Invalid_Authentication_Configuration';
    const SFTP_CONNECTION_SUCCESS = 'Radial_Core_Sftp_Conection_Success';
    const SFTP_CONNECTION_FAILED = 'Radial_Core_Sftp_Connection_Failed';
    ...
    public function __construct()
    {
        ...
        $this->_helper = Mage::helper('radial_core');
    ...
    public function testApiConnection($storeId, $apiKey, $hostname)
    {
        ...
        } catch (Radial_Core_Exception_Api_Configuration $e) {
            return [
                'message' => $e->getMessage(), 'success' => false
            ];
        }
        // need to manually build the URI as the radial_core/helper method doesn't
    ...
    {
        return sprintf(
            Radial_Core_Helper_Data::URI_FORMAT,
            $hostname,
            $major,
            $minor,
            $storeId,
     ...
     * @param  string $hostname
     * @return self
     * @throws Radial_Core_Exception_Api_Configuration If any settings are empty
     */
    protected function _validateApiSettings($storeId, $apiKey, $hostname)
    {
        ...
        if (!empty($invalidSettings)) {
            throw new Radial_Core_Exception_Api_Configuration(
...
REQUEST;
     ...
     * @param  string $key
     * @param  int $port
     * @throws Radial_Core_Exception_Sftp_Configuration
     * @return self
     */
    protected function _validateSftpSettings($host, $username, $key, $port)
    {
        ...
        if (!empty($invalidSettings)) {
            throw new Radial_Core_Exception_Sftp_Configuration(
    ...
    public function testSftpConnection($host, $username, $privateKey, $port)
    {
        ...
        } catch (Radial_Core_Exception_Sftp_Configuration $e) {
            return ['message' => $e->getMessage(), 'success' => false];
        }
        $configMap = Mage::getSingleton('radial_core/config');
        ...
        set_error_handler(function ($errno, $errstr) {
            throw new Radial_Core_Exception_Sftp_Configuration($errstr);
        }, E_USER_NOTICE);
        try {
            $sftp->connect()->login();
        } catch (Radial_Core_Exception_Sftp_Configuration $e) {
