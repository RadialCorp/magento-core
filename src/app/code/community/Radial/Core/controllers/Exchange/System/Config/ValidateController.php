...
class Radial_Core_Exchange_System_Config_ValidateController extends Mage_Adminhtml_Controller_Action
{
    ...
    /** @var Radial_Core_Helper_Validator */
     ...
     *                          - 'validator_helper' => Radial_Core_Helper_Validator
     */
    public function __construct(Zend_Controller_Request_Abstract $request, Zend_Controller_Response_Abstract $response, array $initParams = array())
    {
        ...
        list($this->_validatorHelper) = $this->_checkTypes(
            $this->_nullCoalesce($initParams, 'validator_helper', Mage::helper('radial_core/validator'))
        );
    }
    /**
     * Type checks for __construct's $initParams
     * @param Radial_Core_Helper_Validator $validatorHelper
     * @return mixed[]
     */
    protected function _checkTypes(Radial_Core_Helper_Validator $validatorHelper)
    {
    ...
    public function validateapiAction()
    {
        ...
        $configMap = Mage::getModel('radial_core/config');
        ...
        $apiKey = $this->_validatorHelper->getEncryptedParamOrFallbackValue(
            ...
            Mage::getSingleton('radial_core/config')->getPathForKey('api_key')
        );

        $this->getResponse()->setHeader('Content-Type', 'text/json')
            ->setBody(json_encode(Mage::helper('radial_core/validator')->testApiConnection($storeId, $apiKey, $hostname)));
        return $this;
    }
    /**
    ...
    public function validatesftpAction()
    {
        ...
        $key = $this->_validatorHelper->getSftpPrivateKey(
            ...
            Mage::getSingleton('radial_core/config')->getPathForKey('sftp_private_key')
        );

        $this->getResponse()->setHeader('Content-Type', 'text/json')
            ->setBody(json_encode(Mage::helper('radial_core/validator')->testSftpConnection($host, $username, $key, $port)));
        return $this;
    }
}
