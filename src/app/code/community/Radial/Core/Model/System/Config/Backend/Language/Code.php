 ...
 */
class Radial_Core_Model_System_Config_Backend_Language_Code extends Mage_Core_Model_Config_Data
{
    const INVALID_LANGUAGE_CODE_MESSAGE = 'Radial_Core_System_Config_Language_Code_Invalid';

    /** @var EbayEnteprise_Eb2cCore_Helper_Languages */
    protected $_langHelper;
    /** @var Radial_Core_Helper */
    ...
    protected function _construct()
    {
        ...
        list($this->_langHelper, $this->_coreHelper) = $this->_checkTypes(
            $this->getData('language_helper') ?: Mage::helper('radial_core/languages'),
            $this->getData('core_helper') ?: Mage::helper('radial_core')
        );
    }

    /**
     * @param Radial_Core_Helper_Languages
     * @return array
     */
    protected function _checkTypes(
        Radial_Core_Helper_Languages $langHelper,
    ...
    protected function _validateLanguageCode($langCode)
    {
        if (!$this->_langHelper->validateLanguageCode($langCode)) {
            throw Mage::exception('Radial_Core', $this->_coreHelper->__(self::INVALID_LANGUAGE_CODE_MESSAGE));
        }
        return $this;
    }
}
