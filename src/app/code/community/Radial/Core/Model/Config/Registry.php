 ...
 */
class Radial_Core_Model_Config_Registry
{
    /**
     ...
     * @var Radial_Core_Model_Config_Interface[]
     */
    protected $_configModels = array();

    /**
     * Add a new config settings model to the collection and update the list of known config paths
     * @param Radial_Core_Model_Config_Interface $configModel
     * @return self
     */
    public function addConfigModel(Radial_Core_Model_Config_Interface $configModel)
    {
    ...
    public function __get($name)
    {
        ...
        $configKey = Mage::helper('radial_core')->underscoreWords(substr($name, 0, strlen($name) - 4 * $isFlag));
        try {
