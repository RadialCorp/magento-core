...
class Radial_Core_Model_Api
{
     ...
     */
    const DEFAULT_HANDLER_CONFIG = 'radial_core/api/default_status_handlers/silent';
    /**
     * path to the default configuration for handling status codes loudly.
     */
    const DEFAULT_LOUD_HANDLER_CONFIG = 'radial_core/api/default_status_handlers/loud';
    /**
     * path handler config
     * @var string
     */
    ...
    public function request(
        ...
        if (!$apiKey) {
            $apiKey = Mage::helper('radial_core')->getConfigModel()->apiKey;
        }
        $xmlStr = $doc->C14N();
    ...
    protected function _processResponse(Zend_Http_Response $response, $uri)
    {
        ...
        return Mage::helper('radial_core')->invokeCallBack($callbackConfig);
    }
    /**
     * @see self::_status
     */
    ...
    public function schemaValidate(DOMDocument $doc, $xsdName)
    {
        ...
        if (!$isValid) {
            ...
            throw new Radial_Core_Exception_InvalidXml($msg);
        }
        return $this;
    }
    /**
    ...
    protected function _getHandlerConfig($handlerKey)
    {
        ...
        $config = $this->_getMergedHandlerConfig(
            Mage::helper('radial_core')->getConfigModel()->getConfigData($path)
        );
