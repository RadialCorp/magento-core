...
class Radial_Core_Test_Helper_ValidatorTest extends Radial_Core_Test_Base
{
    /**
    ...
    public function testValidateSettings()
    {
        ...
        // in the constructor.
        $helper = $this->getHelperMock('radial_core/validator', null);
        $this->assertSame(
            $helper,
    ...
    public function testValidateInvalidSettings($storeId, $apiKey, $hostname, $exceptionMessage)
    {
        $this->setExpectedException(
            'Radial_Core_Exception_Api_Configuration',
            $exceptionMessage
        );
        $translationHelper = $this->getHelperMock('radial_core/data', ['__']);
        $translationHelper->expects($this->once())
            ->method('__')
            ->will($this->returnArgument(1));
        $this->replaceByMock('helper', 'radial_core', $translationHelper);
        // Use a mock of the helper being tested to make sure a new instance
        // is generated, preventing (un)mocked dependencies from being cached
        // in the constructor.
        $helper = $this->getHelperMock('radial_core/validator', null);
    ...
    {
        $this->setExpectedException(
            'Radial_Core_Exception_Sftp_Configuration',
            $exceptionMessage
        );
        $translationHelper = $this->getHelperMock('radial_core/data', ['__']);
        $translationHelper->expects($this->once())
    ...
    public function testGetParamOrFallbackValue($params, $configValue, $expectedValue)
    {
        ...
        // Use a mock of the helper being tested to make sure a new instance
            // is generated, preventing (un)mocked dependencies from being cached
            // in the constructor.
        $helper = $this->getHelperMock('radial_core/validator', ['getConfigSource']);
    ...
    {
        $configPath = 'radial_core/api/key';
    ...
    public function testGetSftpPrivateKey($requestParams, $sourceType, $apiKey)
    {
        ...
        $configPath = 'radial_core/feed/filetransfer_sftp_ssh_prv_key';
