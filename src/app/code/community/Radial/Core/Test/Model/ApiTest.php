...
class Radial_Core_Test_Model_ApiTest extends Radial_Core_Test_Base
{
    const MODEL_CLASS = 'Radial_Core_Model_Api';
    ...
    {
        $domDocument = Mage::helper('radial_core')->getNewDomDocument();
    ...
    public function testRequest()
    {
        ...
        $api = $this->getModelMock('radial_core/api', array('_processResponse', '_processException', 'schemaValidate'));
    ...
    public function testRequestProvideApiKey()
    {
        ...
        $api = $this->getModelMock('radial_core/api', array('schemaValidate', '_setupClient', '_processResponse'));
    ...
    public function testZendHttpClientException($request, $apiUri, $xsdName)
    {
        ...
        $api = $this->getModelMock('radial_core/api', array('_setupClient', '_processResponse', '_processException'));
    ...
    {
        $mock = $this->getModelMockBuilder('radial_core/config_registry')
            ->disableOriginalConstructor()
        ...
        $mockConfig = array(
            ...
            array('apiRegion', 'eu'),
            array('apiXsdPath', 'app/code/local/Radial/Core/xsd'),
            array('storeId', 'store-123'),
        );
        $mock->expects($this->any())
            ->method('__get')
            ->will($this->returnValueMap($mockConfig));
        $this->replaceByMock('model', 'radial_core/config_registry', $mock);
    }
    ...
    public function testProcessResponse()
    {
        ...
        $helper = $this->getHelperMock('radial_core/data', array('invokeCallback'));
        $api = $this->getModelMock('radial_core/api', array('_getHandlerConfig', '_getHandlerKey'));
        ...
        $this->replaceByMock('helper', 'radial_core', $helper);
    ...
    public function testGetHandlerConfig()
    {
        ...
        $handlerKey = 'success';
        $configPath = 'radial_core/api/status/handler/testhandler';

        $api = $this->getModelMock('radial_core/api', array('getIsFailureLoud', 'getStatusHandlerConfigPath', '_getMergedHandlerConfig'));
        ...
        $configRegistryMock = $this->getModelMock('radial_core/config_registry', array('getConfigData'));
        $configRegistryMock->expects($this->once())
            ->method('getConfigData')
            ->with($this->identicalTo($configPath))
            ->will($this->returnValue($configArray));
        $this->replaceByMock('model', 'radial_core/config_registry', $configRegistryMock);
    ...
    public function testGetHandlerKey($code, $key)
    {
        $response = $this->getMockBuilder('Zend_Http_Response')
            ...
            ->getMock();
        $api = Mage::getModel('radial_core/api');
    ...
    public function testGetMergedHandlerConfigWithEmptyConfig()
    {
        ...
        $configRegistryMock->expects($this->once())
            ->method('getConfigData')
            ->with($this->identicalTo(Radial_Core_Model_Api::DEFAULT_HANDLER_CONFIG))
            ->will($this->returnValue($defaultConfig));
        $this->replaceByMock('model', 'radial_core/config_registry', $configRegistryMock);

        $api = $this->getModelMock('radial_core/api', array('getHandlerConfigPath'));
    ...
    public function testGetMergedHandlerConfigWithLoudConfig()
    {
        ...
        $configRegistryMock->expects($this->exactly(2))
            ...
            ->will($this->returnValueMap(array(
                array(Radial_Core_Model_Api::DEFAULT_HANDLER_CONFIG, $defaultConfig),
                array(Radial_Core_Model_Api::DEFAULT_LOUD_HANDLER_CONFIG, $loudConfig),
            )));
        $this->replaceByMock('model', 'radial_core/config_registry', $configRegistryMock);
