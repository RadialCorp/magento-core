 ...
 */
class Radial_Core_Test_Model_ConfigRegistryTest extends EcomDev_PHPUnit_Test_Case
{
    /**
    ...
    {
        $stub = $this->getMock('Radial_Core_Model_Config_Abstract');
        ...
        $pathMap = array(
            array('catalog_id', 'radial/core/catalog_id'),
            array('api_key', 'radial/core/api_key'),
            array('test_mode', 'radial/core/test_mode'),
        );
        $stub->expects($this->any())
    ...
    protected function _createAltConfigStub()
    {
        ...
        $pathMap = array(
            array('catalog_id', 'radial/another/catalog_id'),
            array('another_setting', 'radial/another/module/setting'),
        );
        $stub->expects($this->any())
            ->method('getPathForKey')
            ->will($this->returnValueMap($pathMap));
    ...
    {
        $config = Mage::getModel('radial_core/config_registry');
        ...
        $this->assertSame($config->getConfig('catalog_id'), Mage::getStoreConfig('radial/core/catalog_id'));

        // when explicitly passing a storeId, should return value for that store
        $this->assertSame($config->getConfig('api_key', 2), Mage::getStoreConfig('radial/core/api_key', 2));

        // when store id is set on config object, will use that value for the store id
        $config->setStore(2);
        $this->assertSame($config->getConfig('catalog_id'), Mage::getStoreConfig('radial/core/catalog_id', 2));

        // can still use an explicit store id which should override the one set on the store
        $this->assertSame($config->getConfig('api_key', 3), Mage::getStoreConfig('radial/core/api_key', 3));

        // can even explicitly set store id to null
        $this->assertSame($config->getConfig('catalog_id', null), Mage::getStoreConfig('radial/core/catalog_id'));

        // indicate the config is a "flag" to explicitly return a boolean value
        $this->assertSame($config->getConfigFlag('test_mode', 1), Mage::getStoreConfigFlag('radial/core/test_mode', 1));
    }
    ...
    public function testMagicPropConfig()
    {
        ...
        // when no store id is set, should use whatever the default is
        $this->assertSame($config->apiKey, Mage::getStoreConfig('radial/core/api_key'));

        // should be able to get config added by either settings model
        $this->assertSame($config->anotherSetting, Mage::getStoreConfig('radial/another/module/setting'));

        // keys can collide...last added are used
        // this path is in the first config model added and will not be used
        $this->assertNotSame($config->catalogId, Mage::getStoreConfig('radial/core/catalog_id'));
        // this is in the second config model and will be used
        $this->assertSame($config->catalogId, Mage::getStoreConfig('radial/another/catalog_id'));
    }
    ...
    {
        Mage::getModel('radial_core/config_registry')->getConfig('nonexistent_config');
    }
    ...
    {
        Mage::getModel('radial_core/config_registry')->nonexistentConfig;
    }
    /**
    ...
    {
        Mage::getModel('radial_core/config_registry')->someConfig = 'foo';
    }
    ...
    public function testAllPropsReadonly()
    {
        ...
        $this->assertSame(Mage::getStoreConfig('radial/core/catalog_id'), $config->catalogId);
        set_error_handler($handler);
    }
}
