...
class Radial_Amqp_Test_Model_Api_ValidatorTest extends Radial_Core_Test_Base
{
    const QUEUE_NAME = 'q.Event.Name';
    /** @var Radial_Amqp_Helper_Api_Validator */
    protected $_validator;
    /** @var Radial_Amqp_Helper_Data mock */
    ...
    public function setUp()
    {
        ...
        $this->_amqpHelper = $this->getHelperMock('radial_amqp/data');
        $this->_amqpHelper->expects($this->any())
            ->method('getSdkAmqp')
            ->will($this->returnValue($this->_amqpApi));
        $this->_validator = Mage::getModel('radial_amqp/api_validator', array('helper' => $this->_amqpHelper));
    ...
    {
        return array(
            array(false, true, 'Radial_Amqp_Exception_Configuration_Exception'),
            array(true, false, 'Radial_Amqp_Exception_Connection_Exception'),
    ...
    {
        $validator = $this->getModelMock('radial_amqp/api_validator', array('_validateConfiguration', '_validateConnection'));
    ...
    public function testValidateConnection(array $queues = array(), $openSuccess, $connectionSuccess)
    {
        ...
        if (empty($queues)) {
            $this->setExpectedException('Radial_Amqp_Exception_Configuration_Exception');
        // when the connection fails to be made, should throw a connection exception
        } elseif (!$connectionSuccess) {
            $this->setExpectedException('Radial_Amqp_Exception_Connection_Exception');
        }
